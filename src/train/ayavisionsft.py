...
from src.utils.eval_sarcasm import evaluate_aya
...
def main() -> None:
    try:
        ...
        # Use a dedicated adapter output folder for Aya Vision runs.
        adapter_output_dir = cfg["output"]["adapter_output_dir"] or os.path.join(
            cfg["sft"].model_dir, "sft", "aya_vision_8b"
        )
        ...
        eval_collator = AyaVisionSFTCollator(
            processor=processor,
            training=False,
            max_length=cfg["sft"].max_length,
            image_key=cfg["collator"]["image_key"],
        ...
        pre_metrics = evaluate_aya(model, processor, eval_dataset, eval_collator, cfg)
        if run is not None:
        ...
        post_metrics = evaluate_aya(model, processor, eval_dataset, eval_collator, cfg)
        if run is not None:
        ...
        raise RuntimeError(f"Aya Vision SFT failed: {exc}") from exc
