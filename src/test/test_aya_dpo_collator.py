...
from src.collators.aya_dpo_collator import AyaVisionDPOCollator
...
def _format_full_text(
    collator: AyaVisionDPOCollator,
    example: dict,
    processor,
    answer_key: str,
) -> str:
    user_text = collator._user_text(example)
    messages = collator._messages(user_text, collator._normalize_image(example[collator.image_key]))
...
def _print_mask_check(
    ...
    has_assistant_loss = torch.any(labels != -100).item()
...
def main() -> None:
    try:
        ...
        processor = AutoProcessor.from_pretrained(mc.aya_model_name, use_fast=True)
        tok = _get_tokenizer(processor)
        ...
        prompt_text = processor.apply_chat_template(
            collator._messages(
                collator._user_text(first),
                collator._normalize_image(first[collator.image_key]),
            ),
            tokenize=False,
        ...
        raise RuntimeError(f"Aya DPO collator test failed: {exc}") from exc
