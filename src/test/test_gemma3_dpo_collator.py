...
from src.collators.gemma3_dpo_collator import Gemma3VisionDPOCollator
...
def _format_full_text(
    collator: Gemma3VisionDPOCollator,
    example: dict,
    processor,
    answer_key: str,
) -> str:
...
def _prompt_len_from_inputs(
    ...
    prompt_text = processor.apply_chat_template(
        collator._user_message(user_text),
        tokenize=False,
...
def main() -> None:
    try:
        ...
        prompt_len = _prompt_len_from_inputs(collator, first, processor, max_length=2048)
        print(f"prompt_len_from_inputs={prompt_len}")
        ...
        raise RuntimeError(f"Gemma3 DPO collator test failed: {exc}") from exc
