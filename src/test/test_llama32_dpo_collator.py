...
from src.collators.llama32_dpo_collator import Llama32VisionDPOCollator
...
def _format_full_text(
    collator: Llama32VisionDPOCollator,
    example: dict,
    processor,
    answer_key: str,
) -> str:
...
def _assistant_prompt_len_from_input_ids(input_ids: torch.Tensor, processor) -> int:
    ...
    if start == -1:
        raise RuntimeError("assistant header marker not found in input_ids")
...
def main() -> None:
    try:
        ...
        processor = AutoProcessor.from_pretrained(mc.llama3_2vl, use_fast=True)
        tok = _get_tokenizer(processor)
        ...
        raise RuntimeError(f"Llama32 DPO collator test failed: {exc}") from exc
